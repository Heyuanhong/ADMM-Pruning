...
def apply_prune(model, device, percents, prune_type, prune_block):
    ...
    for name, param in model.named_parameters():
        if not any(nd in name for nd in no_decay):
            ...
            if prune_type == "bbs":
                mask = bbs_weight(param, device, percents, prune_block)
...
def train(args, train_dataset, model, tokenizer):
    ...
    # Train!
    logger.info("***** Running training stage 1*****")
            ...
            if (step + 1) % args.gradient_accumulation_steps == 0:
                ...
                if args.local_rank in [-1, 0] and args.logging_steps > 0 and global_step % args.logging_steps == 0:
                    ...
                    if args.local_rank == -1:  # Only evaluate when single GPU otherwise metrics may not average well
                        ...
                        if(results["f1"] > best_dev):
                            best_dev = results["f1"]
                        logger.info("best dev = {}".format(best_dev))


######stage 2
    print("stage 2 ")
    iterative_count = 1.0 
    percents = args.start_percent
    while(percents <= args.end_percent):
        if(args.prune_method == "admm"):
            ...
            optimizer = AdamW(optimizer_grouped_parameters, lr=args.learning_rate, eps=args.adam_epsilon)
            scheduler = WarmupLinearSchedule(optimizer, warmup_steps=args.warmup_steps, t_total=t_total)
            logger.info("***** Running training  stage 2*****")
            logger.info("  Num examples = %d", len(train_dataset))
            logger.info("  Num Epochs = %d", args.num_train_epochs)
            global_step = 0 
            best_dev = 0 
            ...
            for _ in range(int(args.num_train_epochs)):
                ...
                for step, batch in enumerate(train_dataloader):
                    ...
                    if(step % ((t_total / 3) // 10)  == 0) :
                        X = update_X(model)
                        Z = update_Z(X, U, percents=percents)
                        U = update_U(U, X, Z)
    ...
    ######stage 3
        print("stage 3 ")
        mask = apply_prune(model, args.device, percents, args.prune_type, args.prune_block)
        global_step = 0
        tr_loss, logging_loss = 0.0, 0.0
        optimizer_grouped_parameters = [
            {'params': [p for n, p in model.named_parameters() if not any(nd in n for nd in no_decay)],
                ...
                if (step + 1) % args.gradient_accumulation_steps == 0:
                    ...
                    if args.local_rank in [-1, 0] and args.logging_steps > 0 and global_step % args.logging_steps == 0:
                        ...
                        if args.local_rank == -1:  # Only evaluate when single GPU otherwise metrics may not average well
                            evaluate(args, model, tokenizer)
        iterative_count += 1
        if(args.gradual_model == "linear"):
            loop_num = int((args.end_percent - args.start_percent + 0.05) * 10 + 1)
            percents = args.start_percent + 0.1 * (iterative_count - 1)
            if loop_num == iterative_count:
                percents == args.end_percent
...
def main():
    ...
    parser.add_argument('start_percent', tyep=float, default=0 )
    parser.add_argument("--prune_type", type=str, default="random", help="prune type,random or bbs")
    parser.add_argument("--prune_block", type=int, default=64, help="prune block size")
