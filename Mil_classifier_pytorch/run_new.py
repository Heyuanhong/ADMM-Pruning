...
from ori_transformers import (WEIGHTS_NAME, BertConfig,
...
from ori_transformers import AdamW, WarmupLinearSchedule
...
def admm_loss(device, model, Z, U, nll_loss, alpha, rho, l2):
...
def apply_prune(model, device, percent, prune_type, prune_block):
    ...
    for name, param in model.named_parameters():
        if not any(nd in name for nd in no_decay):
            if prune_type == "random":
                mask = prune_weight(param, device, percent)
            if prune_type == "bbs":
                mask = bbs_weight(param, device, percent, prune_block)
    ...
    return dict_mask, dict_one, dict_two
def bbs_weight(t, device, percent, prune_block):
    # Check that the amount of units to prune is not > than the number of
    # parameters in t
    #每个稀疏块的大小
    block_size = int(t.shape[1] / prune_block)
    cur_t = t.view(t.shape[0], -1 ,block_size)  # change the shape of tensor

    tensor_size = cur_t.shape[-1]
    nchannels_toprune = int(round(percent * tensor_size))
    ...
    def make_mask(t,  nchannels_toprune):
        # generate a random number in [0, 1] to associate to each channel
        # prob = torch.rand(nchannels)
        # _, indice = abs(t).topk(nchannels_toprune, dim=2, largest=True)
        # # generate mask for each channel by 0ing out the channels that
        # # got assigned the k = nchannels_toprune lowest values in prob

        ## the old method of bbs
        # threshold = torch.kthvalue(abs(t), k=nchannels_toprune,dim=2).values
        # mask = abs(t)>threshold.unsqueeze(2)

        ## the new method of bbs
        share_index = 1
        t = abs(t.view(t.shape[0], -1, share_index, block_size)).sum(2)
        # print(t.shape)  # test the view
        # print(nparams_toprune)
        threshold = torch.kthvalue(abs(t), k=nchannels_toprune, dim=2).values
        mask = abs(t) > threshold.unsqueeze(2)

        mask = mask.repeat(1, 1, share_index)
        ...
        # unstructured) mask
    mask = make_mask(cur_t, nchannels_toprune)
...
def train(args, train_dataset, model, tokenizer):
    ...
    for _ in range(int(args.num_train_epochs)):
        ...
        for step, batch in enumerate(train_dataloader):
            ...
            loss = outputs[0]  # model outputs are always tuple in ori_transformers (see doc)
            ...
            if (step + 1) % args.gradient_accumulation_steps == 0:
                ...
                if args.local_rank in [-1, 0] and args.logging_steps > 0 and global_step % args.logging_steps == 0 and global_step > t_total * ((args.num_train_epochs -1) / args.num_train_epochs):
                    # Log metrics
                    if args.local_rank == -1:  # Only evaluate when single GPU otherwise metrics may not average well
                        result = evaluate(args, model, tokenizer)
                        if(result["f1"] > best_dev):
                            best_dev = result["f1"]
                        logger.info("base best dev = {}".format(best_dev))
    ...
    while(percent <= args.end_percent):
        ...
        # ADMM 剪枝和 magntidue剪枝的区别在于是否有stage two
            ...
            for _ in range(int(args.num_train_epochs)):
                ...
                for step, batch in enumerate(train_dataloader):
                    ...
                    if args.fp16:
                        ...
                        admmloss = admm_loss(args.device, model, Z, U, loss, args.alpha, args.rho, args.l2)
                        admmloss.backward()
                        torch.nn.utils.clip_grad_norm_(model.parameters(), args.max_grad_norm)
                    ...
                    if (step + 1) % args.gradient_accumulation_steps == 0:
                        ...
                        #     # tokenizer.save_vocabulary(vocab_path=output_dir)
                        #     tokenizer.save_vocabulary(output_dir)
                    if(step % ((t_total / args.num_train_epochs) // 10)  == 0) :
                        #每隔1/10 个周期进行一次admm剪枝参数更新
                        X = update_X(model)
                        Z = update_Z(X, U, percent=percent)
                        U = update_U(U, X, Z)
        ...
        for _ in range(int(args.num_train_epochs)):
            ...
            for step, batch in enumerate(train_dataloader):
                ...
                tr_loss += loss.item()
                if (step + 1) % args.gradient_accumulation_steps == 0:
                    optimizer.prune_step(mask, dict_one, dict_two)
                    scheduler.step()  # Update learning rate schedule
                    model.zero_grad()
                    global_step += 1

                    if args.local_rank in [-1, 0] and args.logging_steps > 0 and global_step % args.logging_steps == 0 and global_step > t_total * ((args.num_train_epochs -2) / args.num_train_epochs):
                        print(" ")
                        # Log metrics
                        if args.local_rank == -1:  # Only evaluate when single GPU otherwise metrics may not average well
                            result = evaluate(args, model, tokenizer)
                            if(result["f1"] > best_dev):
                                best_dev = result["f1"]
                        logger.info("percent = {},best dev = {}".format(percent, best_dev))
...
def load_and_cache_examples(args, task, tokenizer, data_type='train'):
    ...
    #     str(task)))
    # cached_features_file = os.path.join(args.data_dir,'cached_{}_{}_{}'.format(data_type,str(args.max_seq_length),str(task)))
    cached_features_file = os.path.join(args.data_dir,'cached_{}_{}_{}'.format(data_type,str(args.max_seq_length),"milnews"))
...
def main():
    ...
    parser.add_argument("--prune_type", type=str, default="random", help="prune type,random or bbs")
    parser.add_argument("--prune_block", type=int, default=64, help="prune block size, for bbs pruning")
    parser.add_argument("--gradual_model", type=str, default="linear", help="gradual scheml")
    parser.add_argument("--prune_method", type=str, default="magnitude", help="admm or magnitude")
    parser.add_argument(
        "--end_percent",
        default=False,
        type=float,
        help="sparsitry end percent"
    )
    parser.add_argument(
        "--start_percent",
        default=False,
        type=float,
        help="sparsity start percent"
    ) 
    parser.add_argument(
        "--l2",
        default=False,
    ...
    args.output_dir = args.output_dir + '{}'.format(args.model_name_or_path.split("/")[-1])
    if not os.path.exists(args.output_dir):
        os.mkdir(args.output_dir)
    # init_logger(log_file=args.output_dir + '/{}-{}.log'.format(args.model_type, args.task_name))
    init_logger(log_file=args.output_dir + '/{}-{}.log'.format(args.model_type, "milnews"))
    ...
    print(494,args.model_name_or_path)
    ...
    if args.do_train and (args.local_rank == -1 or torch.distributed.get_rank() == 0):
        ...
        tokenizer = tokenizer_class.from_pretrained(args.output_dir, do_lower_case=args.do_lower_case, truncation=True)
        model.to(args.device)
    ...
    if args.do_eval and args.local_rank in [-1, 0]:
        ...
        if args.eval_all_checkpoints:
            print("**********************************/n/n eval_all_checkpoints==True /n/n*****************")
            checkpoints = list(
                os.path.dirname(c) for c in sorted(glob.glob(args.output_dir + '/**/' + WEIGHTS_NAME, recursive=True)))
            logging.getLogger("ori_transformers.modeling_utils").setLevel(logging.WARN)  # Reduce logging
        ...
        for checkpoint in checkpoints:
            ...
            os.remove(os.path.join(checkpoint, "vocab.txt"))
            os.remove(os.path.join(checkpoint, "config.json"))
            os.remove(os.path.join(checkpoint, "pytorch_model.bin"))
            os.remove(os.path.join(checkpoint, "training_args.bin"))
            os.remove(os.path.join(checkpoint, "tokenizer_config.json"))
            os.remove(os.path.join(checkpoint, "special_tokens_map.json"))
        ...
        #delete finetuned model pytorch_model.bin
